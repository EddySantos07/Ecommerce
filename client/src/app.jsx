...
let App = function (props) {
  ...
  var changeReviewSortBy = (sortBy) => {
    ...
    fetchReviews(currentProduct, sortBy).then((data) => {
  ...
  var fetchAPIData = () => {
    ...
    fetchProductDetails(currentProduct).then((data) => {
      setProductName(data.name);
      setCurrentProductDetails(data);
    });

    // related products
    fetchRelatedProducts(currentProduct).then((relatedItemsData) => {
      setRelatedItems(relatedItemsData);
    });

    //Questions and Answers
    fetchQA(currentProduct).then((data) => {
      setProductQA(data);
    });

    //Product Styles
    fetchStyles(currentProduct).then((data) => {
      setProductStyles(data);
    });

    //Product Reviews
    fetchReviews(currentProduct, reviewsSortBy).then((data) => {
      setProductReviews(data);
    });

    //Product Reviews Meta Data
    fetchReviewsMetaData(currentProduct).then((data) => {
    ...
    >
      ...
      <div>
        <div id="header">
          <img
            id="header-logo"
            src="https://i.imgur.com/g8hJeOQ.png"
            alt="cream company logo"
          />
        </div>
